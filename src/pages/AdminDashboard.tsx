  ...
  return (
    <div
      className={cn(
        ...
        color === "yellow" &&
          "border-yellow-400/20 hover:border-yellow-400/50 hover:shadow-[0_8px_32px_-4px_rgba(250,204,21,0.25)]",
        color === "green" &&
          "border-green-500/20  hover:border-green-500/50  hover:shadow-[0_8px_32px_-4px_rgba(34,197,94,0.2)]",
        color === "zinc" &&
          "border-zinc-700/40   hover:border-zinc-500/40   hover:shadow-[0_8px_32px_-4px_rgba(120,120,120,0.15)]",
      )}
    >
      {/* glow blob */}
      <div
  ...
  const links = [
    {
      ...
      themeClasses:
        "hover:border-blue-500/30 hover:shadow-[0_8px_32px_-4px_rgba(59,130,246,0.15)]",
    ...
    },
    {
      ...
      themeClasses:
        "hover:border-purple-500/30 hover:shadow-[0_8px_32px_-4px_rgba(168,85,247,0.15)]",
    ...
    },
    {
      ...
      themeClasses:
        "hover:border-emerald-500/30 hover:shadow-[0_8px_32px_-4px_rgba(16,185,129,0.15)]",
      ...
      {links.map((link, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              onClick={() => shareLink(link.url, link.title, link.desc)}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-white leading-tight group-hover/row:text-yellow-400 transition-colors">
            {ticket.customer_name}
          </p>
          {ticket.people_count > 1 && (
            <p className="text-xs text-zinc-600">{ticket.people_count} أشخاص</p>
          )}
        </div>
      </div>
      <button
...
function ServingBadge({
  ...
  return (
    <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-4 overflow-hidden relative">
      ...
      <p className="text-sm text-zinc-300 font-semibold mb-3 relative z-10">
        {ticket.customer_name}
      </p>
      <Button
        onClick={onFinish}
  ...
  return (
    <div
      className={cn(
        ...
        serving
          ? "border-green-500/30"
          : "border-zinc-800 hover:border-yellow-400/20",
      )}
    >
      {/* Card header */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            className={cn(
              ...
              serving ? "text-green-400" : "text-zinc-500",
            )}
          >
            {serving
              ? "يخدم عميلاً"
              : `${waiting.reduce((acc, t) => acc + (t.people_count || 1), 0)} بالانتظار`}
          </p>
        </div>
        {waiting.length > 0 && (
...
export default function AdminDashboard() {
    ...
    const sub = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const isManual = (payload.new as any)?.user_session_id?.startsWith(
            "manual_",
          );
  ...
  const handleManualTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const insertedTicket = (
        Array.isArray(ticketData) ? ticketData[0] : ticketData
      ) as Ticket;
  ...
  const cancelTicket = async (id: string) => {
    const { error } = await supabase
      ...
      .eq("id", id);
    if (error) toast.error("فشل في إلغاء التذكرة");
    else toast.success("تم إلغاء التذكرة");
  };
  ...
  const finishTicket = async (id: string) => {
    const { error } = await supabase
      ...
      .eq("id", id);
    if (error) toast.error("فشل في إنهاء الخدمة");
    else toast.success("تم إنهاء الخدمة ✓");
  };
  ...
  const getBarberTickets = (barberId: string | null, status: string) =>
    tickets.filter((t) => t.barber_id === barberId && t.status === status);
  const getCurrentServing = (barberId: string) =>
    tickets.find((t) => t.barber_id === barberId && t.status === "serving");
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0a0a0a]" dir="rtl">
            ...
            <div className="flex items-center gap-3">
              {shop.logo_url ? (
                <img
                  src={shop.logo_url}
                  alt={shop.name}
                  className="w-10 h-10 object-contain rounded-xl border border-zinc-800"
                />
              ) : (
              ...
              <div>
                <h1 className="font-black text-white text-base leading-tight truncate max-w-[150px] sm:max-w-[200px]">
                  {shop.name}
                </h1>
              </div>
            </div>
              ...
              >
                <span
                  className={cn(
                    ...
                    shop.is_open ? "bg-green-400 animate-pulse" : "bg-zinc-600",
                  )}
                />
            ...
            <div className="flex items-center justify-center gap-2 flex-1 md:flex-none">
              ...
              >
                {soundEnabled ? (
                  ...
                  <BellOff className="w-5 h-5 md:w-4 md:h-4" />
                )}
              </button>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent
                  className="bg-zinc-950 border border-zinc-800 text-white rounded-[2rem] w-[90vw] max-w-[400px]"
                  dir="rtl"
                >
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-zinc-400 text-right mt-2 font-semibold">
                      هل أنت متأكد أنك تريد تصفير الطابور بالكامل؟ سيتم إلغاء
                      جميع التذاكر الحالية وتفريغ الانتظار ولن تتمكن من التراجع
                      عن هذا الإجراء.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter className="flex-row items-center gap-3 mt-4 sm:justify-start">
                    <AlertDialogCancel className="mt-0 flex-1 rounded-xl border-zinc-800 bg-black/50 text-white hover:bg-white/5 hover:text-white font-bold">
                      إلغاء
                    </AlertDialogCancel>
                    <AlertDialogAction
                      onClick={resetQueue}
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <button className="w-10 h-10 md:w-9 md:h-9 rounded-xl flex items-center justify-center border border-zinc-800 text-zinc-500 hover:text-red-400 hover:border-red-500/30 transition-all bg-zinc-950 hover:bg-zinc-900">
                ...
                >
                  <AlertDialogHeader>
                    <AlertDialogTitle className="font-black text-xl text-white text-right">
                      تسجيل الخروج
                    </AlertDialogTitle>
                    <AlertDialogDescription className="text-zinc-400 text-right mt-2">
                      هل أنت متأكد أنك تريد تسجيل الخروج؟ ستحتاج لتسجيل الدخول
                      مرة أخرى للوصول إلى لوحة التحكم.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter className="flex-row items-center gap-3 mt-4 sm:justify-start">
                    <AlertDialogCancel className="mt-0 flex-1 rounded-xl border-zinc-800 bg-black/50 text-white hover:bg-white/5 hover:text-white">
                      إلغاء
                    </AlertDialogCancel>
                    <AlertDialogAction
                      onClick={async () => {
      ...
      <div className="w-full max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          <StatCard
            ...
            value={waitingCount}
            color="yellow"
            sub={waitingCount > 0 ? "بحاجة للخدمة" : "الطابور فارغ"}
          />
          <StatCard
            label="قيد الخدمة"
            value={servingCount}
            color="green"
            sub={servingCount > 0 ? "حلاقون نشطون" : "لا أحد يُخدم"}
          />
          <StatCard
            label="الحلاقون"
        ...
        <Sheet open={isManualTicketOpen} onOpenChange={setIsManualTicketOpen}>
          <SheetTrigger asChild>
            ...
            >
              ...
              {shop.is_open
                ? "إضافة تذكرة يدوياً"
                : "الصالون مغلق (لا يمكن الإضافة)"}
            </button>
          </SheetTrigger>
          <SheetContent
            side="bottom"
            className="rounded-t-[2rem] h-auto max-h-[92vh] bg-zinc-950 border-zinc-800 p-6"
            dir="rtl"
          >
            <SheetHeader className="pb-5">
              <SheetTitle className="text-center text-xl font-black text-white">
                إضافة تذكرة ✂️
              </SheetTitle>
            </SheetHeader>
            <form onSubmit={handleManualTicket} className="space-y-4">
              <div className="space-y-2">
                <Label className="text-zinc-300 text-sm font-bold">
                  اسم العميل
                </Label>
                <Input
                  value={manualName}
                  ...
                  className="rounded-xl h-12 bg-black border-zinc-700 text-white focus-visible:ring-yellow-400 placeholder:text-zinc-600"
                  required
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="rounded-xl h-12 bg-black border-zinc-700 text-white focus-visible:ring-yellow-400 text-left"
                  dir="ltr"
                  required
                />
              </div>
                    ...
                    {barbers
                      ...
                      .map((b) => (
                        <SelectItem
                          key={b.id}
                          value={b.id}
                          className="focus:bg-yellow-400/10 focus:text-yellow-400 cursor-pointer"
                        >
                          {b.name}
                        </SelectItem>
                      ))}
              ...
              <button
                type="submit"
                disabled={submittingManual || !manualBarber}
                className={cn(
            ...
            {barbers.map((barber) => {
              ...
              const hasTickets = tickets.some(
                (t) =>
                  ...
                  (t.status === "serving" || t.status === "waiting"),
              );
                    ...
                    {(() => {
                      ...
                      return activeTickets.map((t) => {
                        ...
                        return (
                          <div
                            key={t.id}
                            onClick={() => setSelectedTicketDetails(t)}
                            className="flex items-center gap-4 px-5 py-4 hover:bg-zinc-900/50 transition-colors group cursor-pointer"
                          >
                            <div
                              className={cn(
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-black text-white text-sm leading-tight group-hover:text-yellow-400 transition-colors truncate">
                                {t.customer_name}
                              </p>
                  ...
                  <div className="p-6 space-y-5">
                    ...
                    {serving && (
                      <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-6 text-center relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                          ...
                          <p className="text-zinc-300 font-bold text-lg mb-5">
                    ...
                    >
                      {processingBarber === barber.id ? (
                        ...
                        <>
                          <ChevronLeft className="w-7 h-7" /> استدعاء العميل
                          التالي
                        </>
                      )}
                    </button>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="font-black text-white text-lg leading-tight group-hover/bigrow:text-yellow-400 transition-colors">
                                <span className="text-zinc-600 text-base ml-1">
                                  #
                                </span>
                                {t.ticket_number}
                              </p>
      ...
      <Dialog
        ...
        onOpenChange={(o) => !o && setSelectedTicketDetails(null)}
      >
        <DialogContent
          className="bg-zinc-950 border-zinc-800 text-white rounded-2xl w-[90vw] max-w-sm p-6"
          dir="rtl"
        >
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-right text-yellow-400">
              تفاصيل الزبون
            </DialogTitle>
            <DialogDescription className="text-zinc-400 text-sm text-right mt-1">
              ...
              {selectedTicketDetails
                ? getTicketCode(
                    getBarberIndex(selectedTicketDetails.barber_id),
          ...
          {selectedTicketDetails && (
            <div className="space-y-4 mt-2">
              <div className="bg-black/50 p-4 rounded-xl border border-zinc-800/80 items-center justify-between">
                <p className="text-zinc-500 text-xs font-bold mb-1">
                  الاسم الكامل
                </p>
                <p className="text-lg font-black text-white">
                  {selectedTicketDetails.customer_name}
                </p>
              ...
              {selectedTicketDetails.phone_number ? (
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-bold opacity-80 mb-0.5 pointer-events-none">
                      رقم الهاتف (انقر للاتصال)
                    </p>
                    <p
                      className="text-lg font-black tracking-wide pointer-events-none"
                      dir="ltr"
                    >
                      {selectedTicketDetails.phone_number}
                    </p>
