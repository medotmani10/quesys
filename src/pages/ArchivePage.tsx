...
import {
  ...
  Clock, Scissors, User, ArrowRight, Loader2, Filter
} from 'lucide-react';
...
export default function ArchivePage() {
  ...
  const formatDate = (dateString: string) => {
    ...
    return date.toLocaleDateString('ar-EG', {
      ...
      month: '2-digit',
      day: '2-digit',
      numberingSystem: 'latn' // Force Western/French numbers
    ...
    <div className="min-h-[100dvh] bg-zinc-950 relative overflow-hidden">
        ...
        <div className="w-full px-4 md:px-8 lg:px-12 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button
              ...
              className="rounded-xl hover:bg-zinc-900 text-zinc-400 hover:text-white transition-colors h-10 w-10"
            >
              <ArrowRight className="w-6 h-6" />
            </Button>
            <div>
              <h1 className="font-bold text-lg text-white">الأرشيف</h1>
              <p className="text-xs text-yellow-400/80">سجل التذاكر المكتملة والملغية</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="w-full px-4 md:px-8 lg:px-12 py-8 relative z-10">
        ...
        <Card className="rounded-[2.5rem] border border-zinc-800/80 bg-zinc-950/50 backdrop-blur-xl shadow-2xl mb-8">
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              {/* Specific Date Select (Shown only if today is selected, but repurposed to 'any specific date') */}
              <div className={`w-full md:w-1/3 flex items-center gap-3 transition-opacity ${timeFilter === 'today' ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                ...
                <Input
                  ...
                  className="flex-1 rounded-2xl h-14 bg-black/50 border-zinc-800 text-white focus-visible:ring-yellow-400 transition-all hover:border-zinc-700"
                />
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="rounded-[2rem] border border-zinc-800/50 bg-zinc-950/50 backdrop-blur-xl shadow-2xl overflow-hidden group">
            ...
            <CardContent className="p-6 text-center relative z-10">
              ...
              <p className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-500 drop-shadow-lg">{stats.total}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="relative mb-8 group">
          <div className="absolute inset-0 bg-gradient-to-r from-yellow-400/0 via-yellow-400/5 to-yellow-400/0 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
          <Search className="absolute right-5 top-1/2 -translate-y-1/2 w-6 h-6 text-zinc-500 group-hover:text-yellow-400 transition-colors z-10" />
          <Input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="البحث بالاسم، رقم التذكرة، أو رقم الهاتف..."
            className="relative rounded-2xl h-16 pr-14 bg-black/50 border-zinc-800 text-white placeholder:text-zinc-600 focus-visible:ring-yellow-400 focus-visible:border-yellow-400/50 transition-all hover:border-zinc-700 text-lg shadow-2xl"
          />
        </div>
        ...
        <div className="space-y-4">
          {filteredTickets.length === 0 ? (
            ...
            filteredTickets.map((ticket) => (
              <Card
                key={ticket.id}
                className="rounded-[2rem] border border-zinc-800 bg-black/40 backdrop-blur-sm hover:border-yellow-400/30 transition-all hover:shadow-[0_0_20px_rgba(250,204,21,0.05)] group"
              >
                <CardContent className="p-5 sm:p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h4 className="font-bold text-xl text-white group-hover:text-yellow-400 transition-colors">{ticket.customer_name}</h4>
                        <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-zinc-500">
                          <span className="flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded-md">
                            <Scissors className="w-4 h-4 text-yellow-400/70" />
